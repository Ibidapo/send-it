      ...
      request(app)
        ...
        .expect((res) => {
          pool.query('SELECT * FROM parcels', (error, results) => {
            ...
            const { rows } = results;
            if (rows.length > 0) {
              expect(res.body).toEqual({ success: 'Order(s) successfully retrieved', rows });
            } else {
        ...
        .expect((res) => {
          pool.query('SELECT * FROM parcels WHERE parcel_id = $1', [1], (error, results) => {
            if (error || results.rowCount === 0) {
              expect(res.body).toEqual({ error: 'Error occured' });
            } else {
              const { rows } = results;
              expect(res.body).toEqual({ success: 'Order successfully retrieved', rows });
            }
          });
        })
        .end(done);
            ...
            error: [
              {
                ...
                param: 'toAddress',
                value: '',
                msg: 'Recipient Address cannot be empty',
              },
            ],
          });
        })
