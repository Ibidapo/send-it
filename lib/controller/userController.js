      ...
      [email], (error, results) => {
        ...
        return jwt.sign({ email: rows[0].email, exp: Math.floor(Date.now() / 1000) + (30 * 60) },
          process.env.JWT_SECRET, (errs, token) => {
            ...
            emailNotification(email, '[SendIT] Password Reset', 'Hey, \t\nYou requested to reset your password. Please click on the following <a href="https://ibidapo.github.io/send-it/UI/reset.html">link</a>. This link is only valid for the next 30 minutes.');
            return res.status(200).send({ success: 'Kindly check your email for further instructions', token });
          });
      });
  }
