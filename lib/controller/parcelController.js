...
class Parcel {
  ...
  static addParcel(req, res) {
    ...
    return pool.query('INSERT INTO parcels (user_id, origin, destination, parcel_kg, quote, status, present_location, recipient_phone, created_on) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *',
      [userId, origin, destination, weightNum, quoteNum, 'In Transit', presentLocation, toPhone, createdOn],
      (error, results) => {
        if (error) {
        ...
        const { rows } = results;
        return res.status(201).send({ success: 'Order created', parcels: rows[0] });
      });
  }
